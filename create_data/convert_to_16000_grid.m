...
data_file='GRID';
new_data='GRID_16';
raw_path = strcat(orig_path,'\',data_file);
new_path = strcat(orig_path,'\',new_data);
files = dir(raw_path)';
sorted_names=natsort({files.name});
i=0;
for file1=3:length(sorted_names)%1...34
    ...
    if ~exist([strcat(new_path,'\',sorted_names{file1})],'dir')
        mkdir(strcat(new_path,'\',sorted_names{file1}))
    end
    for file2=3:length(sorted_names2)%one
        files3=dir(strcat(raw_path,'\',sorted_names{file1},'\',sorted_names2{file2}));
        ...
        for file3=3:length(sorted_names3)
            ...
            abs(P/Q*Fs-16000)
            ynew = resample(y,P,Q);
            audiowrite(strcat(new_path,'\',sorted_names2{file2},'\',sorted_names3{file3}),ynew,16000);
            clear y Fs
        end
    end
end
